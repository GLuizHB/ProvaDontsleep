...
pygame.mixer.music.load("girlscream.mp3")
...
def calculo_mar(face, p_boca):
    ...
    # Alarme quando a boca abrir
    if mar > mar_limiar and not pygame.mixer.music.get_busy():
        pygame.mixer.music.play(-1)  # Toca o som continuamente
    elif mar <= mar_limiar and pygame.mixer.music.get_busy():
    ...
    while cap.isOpened():
        ...
        try:
            for face_landmarks in saida_facemesh.multi_face_landmarks:
                ...
                cv2.putText(frame, f"MAR: {round(mar, 2)} {'Boca aberta' if mar >= mar_limiar else 'Boca fechada'}", (1, 50),
                            cv2.FONT_HERSHEY_DUPLEX,
                            0.9, (255, 255, 255), 2)
        ...
        if cv2.waitKey(10) & 0xFF == ord('c') or cv2.waitKey(10) & 0xFF == ord ('C'):
                        break
